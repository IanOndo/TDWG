...
#'
#' Tests whether or not the species occurrence locations are distributed over its native range.
#'
...
#' @export
hasNativeDistrib <- function(point_data,
                             species_name,
                             species_id = NULL,
                             point_fraction = 0.50,
                             unit_fraction = NULL,
  ...
  test_native = test_point_fraction
  ...
  if(!is.null(unit_fraction)){
    ...
    test_native = test_native & test_unit_fraction
  }
  ...
  if(!is.null(range_filling)){
    ...
    test_native = test_native & test_range_filling
  }
  ...
  if(cleanOcc && !is.null(tdwg_info$cleaned_point_data)){
      ...
      return(
        SpatialPointsDataFrame(	coords=tdwg_info$cleaned_point_data[,coordHeaders],
                                data=data.frame(species=if(recursive) rep(species_name, times= nrow(tdwg_info[[4]])) else species_name,
                                                ...
                                                hasNativeDistrib=if(recursive) rep(test_native,times=nrow(tdwg_info[[4]])) else test_native,
    ...
    return(list(hasNativeDistrib=test_native, crs=tdwg_info$crs, cleaned_point_data=tdwg_info$cleaned_point_data))
  }
  return(test_native)
}
