...
#' @export
hasIntroducedDistrib <- function(point_data,
                                 species_name,
                                 species_id = NULL,
                                 point_fraction = 0.50,
                                 unit_fraction = NULL,
  ...
  test_introduced = test_point_fraction

  # test if the proportion of twdg units condition is satisfied
  if(!is.null(unit_fraction)){
    if(recursive)
    ...
    test_introduced = test_introduced & test_unit_fraction
  }
  # test if the coverage of twdg units condition is satisfied
  if(!is.null(range_filling)){
    if(recursive)
    ...
    test_introduced = test_introduced & test_range_filling
  }
  ...
  if(cleanOcc && !is.null(tdwg_info$cleaned_point_data)){
    ...
    return(list(hasIntroducedDistrib=test_introduced, crs=tdwg_info$crs, cleaned_point_data=tdwg_info$cleaned_point_data))
  }
  return(test_introduced)
}
