...
#' @param force.output A logical. Should the initial point dataset be returned in case all points have been removed by the cleaning ? Default is FALSE.
...
#' @return A two-column data.frame or a sf object with point coordinates cleaned by the species geographic range defined by the TDWG and according to specified criteria.
...
coordinatesRangeCleaner <- function(point_data,
  ...
  tmp.args    <- c("",'point_data','species_name','species_id','status','initial_level','backbone','force.output','sf','verbose','use_name_matching','full_data')
