...
#'
#' Clean the species occurrence locations not occurring within its introduced range.
#'
...
#' @export
cleanByIntroducedDistrib <- function(point_data,
                                     species_name,
                                     species_id = NULL,
                                     initial_level = 2,
                                     backbone='wcvp',
  ...
  tmp.args    <- c("",'point_data','species_name','species_id','initial_level','backbone','force.output','sf','verbose','use_name_matching','full_data')
  ...
  IntroducedDistrib	<- eval(call.tmp, parent.frame())

  #-----------------------------
  #= Non available distribution
  #-----------------------------
  if(!inherits(IntroducedDistrib,"list")){
    if(verbose)
  ...
  #------------------------------
  #= Non introduced distribution
  #------------------------------
  # if(!IntroducedDistrib$hasIntroducedDistrib){
  # if(verbose)
  # warning(paste0("Distribution data for ",species_name," are not distributed over its introduced geographic range according to the TDWG database and the given criteria."));flush.console()
  # if(force.output){
  # if(verbose)
  # warning("Returning initial point dataset.");flush.console()
  # return(point_data)
  ...
  # if(sp){
  # return(
  # SpatialPointsDataFrame(	coords = IntroducedDistrib$cleaned_point_data,
  # data = data.frame(species=species_name,
  # hasIntroducedDistrib=IntroducedDistrib$hasIntroducedDistrib
  # ),
  # proj4string=IntroducedDistrib$crs
  # )
  # )
  # }
  return(IntroducedDistrib$cleaned_point_data)
}
