...
#' @param exclude_doubtful Optional. A logical. Should the regions flagged as 'doubtful' be excluded from the search ? Default is TRUE.
...
#' @export
get_geocode <- function(species_name=NULL,
                        species_id=NULL,
                        status='native',
                        level=2,
  ...
  if(is.null(species_id)){
    ...
    best_guess 	<- switch(backbone, wcvp=unique(kew_checklist[species_name][["taxon_name"]]), unique(kew_checklist[species_name][["acc_full_name_without_family"]]))
  }else{
    best_guess 	<- switch(backbone, wcvp=unique(subset(kew_checklist, ipni_id %in% species_id | plant_name_id %in% species_id)[["taxon_name"]]),
                          unique(subset(kew_checklist, ipni_id %in% species_id | plant_name_id %in% species_id)[["acc_full_name_without_family"]])) 
    use_sp_id =TRUE
  }

  if(anyNA(best_guess)){	# try last updates from api ipni
    results <- queryDistribution(species_name,status=ifelse(status=='native','natives',status),use_acc_name=use_name_matching)
    if(is.null(results)) return(NULL)
    geo_codes <- switch(status, 'introduced' = try(kew_lookup_table[area_code_l3 %in% results$tdwgCode][introduced==1][location_doubtful!=as.numeric(exclude_doubtful)][!continent %in% exclude][[geo_level]],silent=TRUE),
                        'native' = try(kew_lookup_table[area_code_l3 %in% results$tdwgCode][introduced==0][location_doubtful!=as.numeric(exclude_doubtful)][!continent %in% exclude][[geo_level]],silent=TRUE),
                        'both' = try(kew_lookup_table[area_code_l3 %in% results$tdwgCode][location_doubtful!=as.numeric(exclude_doubtful)][!continent %in% exclude][[geo_level]],silent=TRUE))
  ...
  if(length(best_guess)>1){
    db_ids <-  ifelse(best_guess=='Unplaced Unplaced', 
                      switch(backbone, 
                             wcvp=kew_checklist[species_name][["plant_name_id"]],
                             kew_checklist[species_name][["db_id"]]),
                      if(use_sp_id) switch(backbone,
                                           wcvp=subset(kew_checklist, ipni_id %in% species_id | plant_name_id %in% species_id)[["accepted_plant_name_id"]],
                                           subset(kew_checklist, ipni_id %in% species_id | db_id %in% species_id)[["accepted_db_id"]]
                                           )
                      else kew_checklist[species_name][[switch(backbone, wcvp="accepted_plant_name_id","accepted_db_id")]]
                      )
    ...
    db_id 			<- if(best_guess=='Unplaced Unplaced') switch(backbone, wcvp=kew_checklist[species_name][["plant_name_id"]],kew_checklist[species_name][["db_id"]]) else if(use_sp_id) switch(backbone, wcvp=subset(kew_checklist, ipni_id %in% species_id | plant_name_id %in% species_id)[["accepted_plant_name_id"]],subset(kew_checklist, ipni_id %in% species_id | db_id %in% species_id)[["accepted_db_id"]]) else kew_checklist[species_name][[switch(backbone, wcvp="accepted_plant_name_id","accepted_db_id")]]
    if(all(is.na(db_id))){
      Sys.sleep(time_delay) # sleep before trying to access api server
      results <- queryDistribution(species_name,status=ifelse(status=='native','natives',status),use_acc_name=use_name_matching)
      if(is.null(results)) return(NULL)
    ...
    if(length(has_distrib)>0L){
      geo_codes <- switch(status, 'introduced' = try(kew_lookup_table[db_id][introduced==1][location_doubtful!=as.numeric(exclude_doubtful)][!continent_code_l1 %in% exclude][[geo_level]],silent=TRUE),
                          'native' = try(kew_lookup_table[db_id][introduced==0][location_doubtful!=as.numeric(exclude_doubtful)][!continent %in% exclude][[geo_level]],silent=TRUE),
                          'both' = try(kew_lookup_table[db_id][location_doubtful!=as.numeric(exclude_doubtful)][!continent %in% exclude][[geo_level]],silent=TRUE))
      if(inherits(geo_codes,'try-error') || length(geo_codes)==0L || all(is.na(geo_codes))){ # try last updates from the api
        Sys.sleep(time_delay) # sleep before trying to access api server
        results <- queryDistribution(species_name,status=ifelse(status=='native','natives',status),use_acc_name=use_name_matching)
        if(is.null(results)) return(NULL)
